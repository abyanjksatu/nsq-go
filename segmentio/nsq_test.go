...
var nsqd = []string{"localhost:4151"}
var nsqlookup = []string{"localhost:4161"}
...
func TestRequeue(t *testing.T) {
	c := &nsq.Client{Address: "localhost:4151"}
...
func TestDrainAndRequeueOnStop(t *testing.T) {
	p, _ := nsq.NewProducer(nsq.ProducerConfig{
	...
	consumer2, _ := nsq.NewConsumer(nsq.ConsumerConfig{
		Topic:        "test-stop-requeue",
		Channel:      "foo",
		Address:      "localhost:4150",
		DialTimeout:  time.Second * 60,
